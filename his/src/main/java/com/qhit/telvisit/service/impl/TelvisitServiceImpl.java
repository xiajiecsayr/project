...
public class TelvisitServiceImpl  implements ITelvisitService {
    ...
    @Override
    public List<Telvisit> find(Integer cid) {
        String sql = "SELECT * from telvisit t JOIN companyinfo c ON t.cid=c.cid where t.cid='"+cid+"' ORDER BY t.teltime DESC";
        return dao.freeFind(sql);
    }

    @Override
    public  List<Telvisit> findAdd(Integer cid) {
